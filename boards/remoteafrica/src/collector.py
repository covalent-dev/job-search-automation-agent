...
class JobCollector:
    ...
    def _parse_json_ld_job(self, payload: dict) -> dict:
    ...
    def _extract_job_from_detail(self, url: str) -> Optional[Job]:
        if not url:
            return None
        if not self.detail_salary_page:
            self.detail_salary_page = self.context.new_page()
            self.detail_salary_page.set_default_timeout(self.config.get_page_timeout())
            self.detail_salary_page.set_default_navigation_timeout(
                self.config.get_navigation_timeout()
            )

        page = self.detail_salary_page
        if not self._safe_goto(url, page=page):
            return None

        # Try JSON-LD JobPosting first
        json_ld_blocks = page.query_selector_all("script[type='application/ld+json']")
        for block in json_ld_blocks:
            try:
                raw = block.inner_text().strip()
                if not raw:
                    continue
                data = json.loads(raw)
    ...
    def _safe_goto(self, url: str, page: Optional[Page] = None) -> bool:
    ...
    def collect_jobs(self, query: SearchQuery) -> List[Job]:
        ...
        if "remoteafrica" in self.config.get_job_boards():
            return self._collect_remoteafrica_jobs(query)
    ...
    def _collect_remoteafrica_jobs(self, query: SearchQuery) -> List[Job]:
        jobs = []
        seen_links = set()
        max_pages = self.config.get_max_pages()
        unlimited_pages = max_pages <= 0
        ...
        while True:
            ...
            url = self._build_search_url(query, page_index=page_index)
            page_label = f" (page {page_index + 1}/∞)" if unlimited_pages else f" (page {page_index + 1}/{max_pages})"
            logger.info("Remote4Africa list: %s%s", query, page_label)
            print(f"\n🔍 Remote4Africa: {query}{page_label}")

            if not self._safe_goto(url):
                logger.warning("Failed to load Remote4Africa list page")
            ...
            if not unique_links:
                ...
                logger.warning("No job links found; saved debug page")
            ...
            if not unlimited_pages:
                ...
                if not self.page.query_selector(next_selector):
                    logger.info("No next page link; stopping pagination")
                    break
